...
export class QueueService implements IQueueService {
  ...
  constructor(@inject(dependencies.logger) logger: ILoggerService) {
    this._logger = logger;
  }
  ...
  init() {
    try {
      ...
      this._logger.log(
        LogLevels.error,
        `Error creating queue ${this.queueName}: ${e.message}`
      );
    }
  }
  ...
  async enqueueAll(queueItems: any[]) {
    ...
    const errors = await this._queue.saveAll(
      queueItems.map((item) => this._queue.createJob(item))
    );
